    ...
    <div class="space-y-6" x-data="hotspotUsersManager()">
        ...
        <template x-if="filteredUsers.length === 0">
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center shadow-xs">
                ...
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 max-w-sm mx-auto">Tambahkan user manual, sinkronkan dari router, atau generate batch voucher.</p>
                <div class="mt-4 flex items-center justify-center gap-2">
                    <button @click="syncUsers()" class="px-3.5 py-2 text-xs font-medium rounded-lg bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300">
                        Sinkron dari Router
                    </button>
                    ...
                    <a href="{{ route('hotspot.generate') }}" class="px-3.5 py-2 text-xs font-medium rounded-lg bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300">
                        Generate Massal
                    </a>
                </div>
            </div>
                            ...
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                                ...
                                <td class="py-3 px-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <button @click="editUser(user)" class="px-2 py-1 rounded text-xs font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
                                            Edit
                                        </button>
                                        <button @click="deleteUser(user)" class="px-2 py-1 rounded text-xs font-medium text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/50 transition-colors">
                                            Hapus
                                        </button>
                                    </div>
                                </td>
        ...
        <x-modal name="hotspot-user-modal" title="Form User Hotspot">
            <form @submit.prevent="saveUser()" class="space-y-4 text-xs">
                <div>
                    ...
                    <input type="text" x-model="userForm.username" required class="w-full px-3 py-2 rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100">
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="text" x-model="userForm.uptime_limit" placeholder="e.g. 3h, 24h, 30d" class="w-full px-3 py-2 rounded-lg bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-100">
                    </div>
                </div>
                <div>
    ...
    <script>
        function hotspotUsersManager() {
            return {
                ...
                userForm: { id: null, username: '', password: '', profile_id: '', uptime_limit: '3h', comment: '' },
                get filteredUsers() {
