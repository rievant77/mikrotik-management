            ...
            <div class="flex-1 overflow-y-auto px-4 py-5 space-y-6">
                ...
                <div>
                    ...
                    <nav class="space-y-1">
                        ...
                        <a href="{{ route('vouchers.index') }}" class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('vouchers.index') ? 'bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-zinc-100' }}">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="lg:pl-64 flex flex-col flex-1 min-w-0 pb-16 lg:pb-0">
            ...
            <header class="h-16 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-20 transition-colors no-print">
                <div class="h-full px-4 sm:px-6 flex items-center justify-between gap-4">
                    ...
                    <!-- Right Controls: Status Badge, Shift, Dark Mode, Profile & Logout -->
                    <div class="flex items-center gap-2 sm:gap-3" x-data="{ userMenuOpen: false, isSyncing: false }">
                        <!-- Quick Router Sync Button -->
                        <button 
                            @click="isSyncing = true; fetch('{{ route('settings.router.sync') }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' } }).then(r => r.json()).then(data => { isSyncing = false; if(data.success) { window.showToast(data.message, 'success'); setTimeout(() => window.location.reload(), 1000); } else { window.showToast(data.message, 'error'); } }).catch(e => { isSyncing = false; window.showToast('Gagal sinkronisasi router', 'error'); });"
                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                            :disabled="isSyncing"
                            title="Sinkronisasi Data Router Sekarang"
                        >
                            <svg class="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-400" :class="{ 'animate-spin': isSyncing }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                            <span class="hidden sm:inline" x-text="isSyncing ? 'Syncing...' : 'Sync Router'"></span>
                        </button>
                        ...
                        <div class="relative pl-2 border-l border-zinc-200 dark:border-zinc-800">
                            <button 
                            ...
                            <div 
                                ...
                                class="absolute right-0 mt-2 w-56 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-lg py-1.5 z-50 text-xs"
                                style="display: none;"
                            >
                                <div class="px-4 py-2.5 border-b border-zinc-100 dark:border-zinc-800">
                                    <div class="font-bold text-zinc-900 dark:text-zinc-100">{{ auth()->user()->name ?? 'Administrator' }}</div>
                                    <div class="text-[11px] text-zinc-400 truncate">{{ auth()->user()->email ?? 'admin@mikrotik.lan' }}</div>
                                    <span class="mt-1 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300">
                                        Superadmin
                                    </span>
                                </div>
                                ...
                                <form action="{{ route('logout') }}" method="POST" class="border-t border-zinc-100 dark:border-zinc-800 pt-1">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center gap-2 px-4 py-2 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 text-left font-medium">
